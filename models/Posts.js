  ...
  GetPosts: async (req, res) => {
    try {
      ...
      if (search) {
        ...
        const sqlQuery = `SELECT *, users.u_name FROM posts
              INNER JOIN users ON posts.p_user=users.u_id
              WHERE posts.p_operation=? AND (posts.p_title
              LIKE ? OR posts.p_text LIKE ?)
